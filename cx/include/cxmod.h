...
bool  is_root(void);
bool  iface_name_safe(const char *iface);      /* validate iface name is safe for shell */
int   find_tool(const char *name, char *out, size_t outlen); /* locate binary in PATH/sbin */
...
int diagnose_softirq(cxmod_report_t *report);  /* CPU softirq saturation check */
int diagnose_ringbuf(cxmod_report_t *report);  /* NIC ring buffer fill check */
