...
/* Resolved at first use; empty = not found yet */
static char s_tc[CXMOD_PATH_MAX]  = "";
static char s_ip[CXMOD_PATH_MAX]  = "";

static const char *tc_bin(void)
{
    if (s_tc[0] == '\0') {
        if (find_tool("tc", s_tc, sizeof(s_tc)) != CXMOD_OK)
            strncpy(s_tc, "tc", sizeof(s_tc) - 1); /* best-effort fallback */
    }
    return s_tc;
}
...
{
    if (s_ip[0] == '\0') {
        if (find_tool("ip", s_ip, sizeof(s_ip)) != CXMOD_OK)
...
int qos_apply(const char *iface, uint32_t bw_mbit, bool dry_run)
{
    ...
    snprintf(cmd, sizeof(cmd),
             "%s qdisc del dev %s root 2>/dev/null || true", tc_bin(), iface);
    run_cmd(cmd, dry_run);

    /* 2. Root HTB qdisc (default to BULK class 1:30) */
    snprintf(cmd, sizeof(cmd),
             "%s qdisc add dev %s root handle 1: htb default 30", tc_bin(), iface);
    ...
    snprintf(cmd, sizeof(cmd),
             ...
             "rate %s ceil %s", tc_bin(), iface, root_rate, root_rate);
    run_cmd(cmd, dry_run);
    ...
    for (int i = 0; i < NCLASSES; i++) {
        ...
        snprintf(cmd, sizeof(cmd),
                 "%s class add dev %s parent 1:1 classid 1:%d htb "
                 "rate %ukbit ceil %ukbit burst %s prio %d",
                 tc_bin(), iface, c->minor, rate_kbps, ceil_kbps, c->burst, c->prio);
        run_cmd(cmd, dry_run);

        /* SFQ within each leaf */
        snprintf(cmd, sizeof(cmd),
                 "%s qdisc add dev %s parent 1:%d handle %d: sfq perturb 10",
                 tc_bin(), iface, c->minor, c->minor);
        run_cmd(cmd, dry_run);
    }
    ...
    for (int i = 0; i < NCLASSES; i++) {
        ...
        for (int p = 0; p < c->nports && c->ports[p]; p++) {
            ...
            /* TCP dport */
            snprintf(cmd, sizeof(cmd),
                     "%s filter add dev %s parent 1: protocol ip "
                     "prio %d u32 match ip protocol 6 0xff "
                     "match ip dport %d 0xffff flowid 1:%d",
                     tc_bin(), iface, c->prio + 1, port, c->minor);
            run_cmd(cmd, dry_run);
...
{
    if (iface && !iface_name_safe(iface)) {
        fprintf(stderr, "Invalid interface name: %s\n", iface);
        return CXMOD_ERR;
    }
    ...
    char cmd[256];
    if (iface)
        snprintf(cmd, sizeof(cmd), "%s -s qdisc show dev %s", tc_bin(), iface);
    else
        snprintf(cmd, sizeof(cmd), "%s -s qdisc show", tc_bin());
    int rc = system(cmd);
    if (WEXITSTATUS(rc) != 0)
