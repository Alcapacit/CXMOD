...
static int read_offload_ethtool(const char *iface, cxmod_offload_t *off)
{
    ...
    snprintf(cmd, sizeof(cmd), "ethtool -k %.15s 2>/dev/null", iface);
...
int offload_show(const char *iface)
{
    ...
    bool has_ethtool = (system("ethtool --version >/dev/null 2>&1") == 0);
    if (!has_ethtool) {
...
int offload_set(const char *iface, const char *feature, bool enable, bool dry_run)
{
    ...
    snprintf(cmd, sizeof(cmd),
             "ethtool -K %s %s %s", iface, feature, enable ? "on" : "off");
    if (dry_run) {
        printf("  [DRY-RUN] %s\n", cmd);
        return CXMOD_OK;
    }
